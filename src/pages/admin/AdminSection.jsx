...
function ItemForm({ section, initial = {}, onSave, onCancel }) {
  ...
  return (
    <div className="glass rounded-xl p-5 space-y-3" style={{ border: "1px solid var(--border-hard)" }}>
        ...
        <div key={key}>
          ...
          {type === "textarea" ? (
            <textarea rows={3} value={form[key]}
              ...
              className="w-full px-3 py-2 rounded-lg text-sm mono outline-none resize-none"
    ...
    <div className="space-y-4">
      ...
      {items.map(item => (
        <div key={item.id} className="glass rounded-xl p-5"
          ...
          {editing === item.id ? (
            <ItemForm section={section} initial={item}
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  {item.featured && <span className="mono text-xs px-2 py-0.5 rounded" style={{ background: "var(--glow)", color: "var(--accent)", border: "1px solid var(--border)" }}>featured</span>}
                  {item.category && <span className="mono text-xs px-2 py-0.5 rounded" style={{ background: "var(--glow)", color: "var(--muted)", border: "1px solid var(--border)" }}>{item.category}</span>}
                  {item.level && <span className="mono text-xs px-2 py-0.5 rounded" style={{ background: "var(--glow)", color: "var(--muted)", border: "1px solid var(--border)" }}>{item.level}</span>}
                  {item.type && <span className="mono text-xs px-2 py-0.5 rounded" style={{ background: "var(--glow)", color: "var(--muted)", border: "1px solid var(--border)" }}>{item.type}</span>}
                  {!item.visible && <span className="mono text-xs px-2 py-0.5 rounded" style={{ background: "rgba(255,80,80,0.1)", color: "#f87171" }}>hidden</span>}
                </div>
                ...
                {item.tech && <p className="mono text-xs mt-1" style={{ color: "var(--accent)" }}>{item.tech.join(", ")}</p>}
              </div>
      ...
      {items.length === 0 && !adding && (
        <p className="mono text-xs" style={{ color: "var(--muted)" }}>No {section} yet. Add one above.</p>
      )}
    </div>
  );
}